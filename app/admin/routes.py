...
from ..models import Admin, Team, Character, GameSession, GameEvent, MinigameFolder, GameRound, QuestionResponse, FieldConfiguration, db
...
# FELD-KONFIGURATION IMPORTS
from app.admin.field_config import (
...
@login_required
def manage_fields():
    """Übersicht aller Feld-Konfigurationen"""
    if not isinstance(current_user, Admin):
        return redirect(url_for('main.index'))
    ...
    try:
        ...
        field_counts = preview_data['counts']
        ...
        flash('Fehler beim Laden der Feld-Konfigurationen.', 'danger')
        return redirect(url_for('admin.admin_dashboard'))

@admin_bp.route('/edit_field/<int:config_id>', methods=['GET', 'POST'])
@login_required
def edit_field(config_id):
    """Einzelne Feld-Konfiguration bearbeiten"""
    if not isinstance(current_user, Admin):
        return redirect(url_for('main.index'))
    
    try:
        config = FieldConfiguration.query.get_or_404(config_id)
        form = FieldConfigurationForm(field_type=config.field_type, obj=config)
        
        # Pre-populate fieldspezifische Felder
        if request.method == 'GET':
            config_data = config.config_dict
            
            # Katapult-Felder
            if config.field_type in ['catapult_forward', 'catapult_backward']:
                form.min_distance.data = config_data.get('min_distance', 3)
                form.max_distance.data = config_data.get('max_distance', 5)
            
            # Sperren-Feld
            elif config.field_type == 'barrier':
                target_numbers = config_data.get('target_numbers', [4, 5, 6])
                form.target_numbers.data = ','.join(map(str, target_numbers))
            
            # Bonus-Feld
            elif config.field_type == 'bonus':
                form.bonus_type.data = config_data.get('bonus_type', 'extra_dice')
            
            # Fallen-Feld
            elif config.field_type == 'trap':
                form.trap_effects.data = config_data.get('trap_effects', [])
            
            # Ereignis-Feld
            elif config.field_type == 'chance':
                form.chance_events.data = config_data.get('events', [])
            
            # Feste Positionen
            if config.frequency_type == 'fixed_positions':
                positions = config_data.get('positions', [])
                form.fixed_positions.data = ','.join(map(str, positions))
        
        if form.validate_on_submit():
            try:
                # Aktualisiere die Konfiguration
                updated_config = update_field_config(config_id, form.data)
                
                db.session.commit()
                flash(f"Feld-Konfiguration '{config.display_name}' erfolgreich aktualisiert.", 'success')
                return redirect(url_for('admin.manage_fields'))
                
            except Exception as e:
                db.session.rollback()
                current_app.logger.error(f"Fehler beim Aktualisieren der Feld-Konfiguration: {e}", exc_info=True)
                flash('Fehler beim Speichern der Feld-Konfiguration.', 'danger')
        
        # Zusätzliche Template-Daten
        field_templates = get_field_type_templates()
        ...
        current_app.logger.error(f"Fehler beim Laden der Feld-Konfiguration {config_id}: {e}", exc_info=True)
        flash('Feld-Konfiguration nicht gefunden.', 'danger')
        return redirect(url_for('admin.manage_fields'))

@admin_bp.route('/field_preview')
@login_required
def field_preview():
    """Vorschau der Feld-Verteilung auf dem Spielbrett"""
    if not isinstance(current_user, Admin):
        return redirect(url_for('main.index'))
    ...
    try:
        ...
        # Standard-Werte für Vorschau
        max_fields = int(request.args.get('max_fields', 73))
        show_field_numbers = request.args.get('show_field_numbers', 'on') == 'on'
        show_field_types = request.args.get('show_field_types', 'on') == 'on'
        show_statistics = request.args.get('show_statistics', 'on') == 'on'
        highlight_conflicts = request.args.get('highlight_conflicts', 'on') == 'on'
        ...
        # Konflikt-Analyse
        conflicts = validate_field_conflicts() if highlight_conflicts else []
        ...
        current_app.logger.error(f"Fehler beim Generieren der Feld-Vorschau: {e}", exc_info=True)
        flash('Fehler beim Generieren der Feld-Vorschau.', 'danger')
        return redirect(url_for('admin.manage_fields'))

@admin_bp.route('/api/field-colors')
@login_required
def field_colors_api():
    """API für Farb-Zuordnungen (für Frontend-Integration)"""
    if not isinstance(current_user, Admin):
    ...
    try:
        ...
        current_app.logger.error(f"Fehler beim Laden der Farb-Zuordnungen: {e}", exc_info=True)
        return jsonify({
            "success": False,
            "error": "Fehler beim Laden der Farb-Zuordnungen"
        }), 500
...
def import_export_fields():
        ...
        if form.import_submit.data and form.validate_on_submit():
            try:
                ...
                if result['imported_count'] > 0:
                    flash(f"{result['imported_count']} Feld-Konfigurationen erfolgreich importiert.", 'success')
                    
                    # Initialisiere Standard-Konfigurationen falls nötig
                    FieldConfiguration.initialize_default_configs()
                else:
                    flash('Keine Konfigurationen importiert.', 'warning')
            ...
            try:
                if reset_to_default_configurations():
                    flash('Alle Feld-Konfigurationen auf Standard-Werte zurückgesetzt.', 'success')
                else:
    ...
    return render_template('admin/import_export_fields.html', form=form)
...
def bulk_edit_fields():
    ...
    if form.validate_on_submit():
        try:
            ...
            configs = FieldConfiguration.query.filter(FieldConfiguration.id.in_(selected_field_ids)).all()
            
            if not configs:
                flash('Keine Felder ausgewählt.', 'warning')
            ...
            for config in configs:
                if action == 'enable':
                    ...
                    if form.new_frequency_type.data and form.new_frequency_value.data is not None:
                    ...
                    # Nur Custom-Felder löschen, nicht Standard-Felder
                    if config.field_type not in ['start', 'goal', 'normal']:
                        db.session.delete(config)
                        updated_count += 1
            
            db.session.commit()
            flash(f"Massen-Bearbeitung abgeschlossen. {updated_